                        ...
                        for index1x=1:length(index1)
                            for index2x=1:length(index2)
                                ...
                                switch dealmethod
                                    ...
                                    case 2 %左右翻转处理
                                        bbgt=[WIDTH-classifier_bbx{class}(index2(index2x),5),classifier_bbx{class}(index2(index2x),4),WIDTH-classifier_bbx{class}(index2(index2x),3),classifier_bbx{class}(index2(index2x),6)];
                                    case 3 %上下翻转处理
                                        bbgt=[classifier_bbx{class}(index2(index2x),3),HEIGHT-classifier_bbx{class}(index2(index2x),6),classifier_bbx{class}(index2(index2x),5),HEIGHT-classifier_bbx{class}(index2(index2x),4)];
                                end
                                flag = IOU(bb,bbgt,ovmax);
                                flagsum(dealmethod) = flagsum(dealmethod) + flag;
                            end
                        end
                    end
                    [flagsummax,flagindex] = max(flagsum);
                    if flagsummax/(length(index1)*length(index2)) >= ovratio % 同一类内的IOU面积大于阈值的个数超过ovratio则判断该类有重叠
