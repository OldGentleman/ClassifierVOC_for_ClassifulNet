function imshowperclass4(classifier_absnumdeal)
VOCopts = VOCinit();
...
for i=1:length(classifier_absnumdeal)
    ...
    for j=1:length(classifier_absnumdeal{i})
        ...
        if num==11
            ...
            saveas(h,strcat('imshowperclass2/',num2str(i),'-',num2str(j),'-',num2str(j),'.jpg'))
            sumsumplot=[];
            num=1;
        elseif j==length(classifier_absnumdeal{i})
            22222222222222
            [h,l]=size(sumplot);
